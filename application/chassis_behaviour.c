...
/**
  * @brief          logical judgement to assign "chassis_behaviour_mode" variable to a mode
...
void chassis_behaviour_mode_set(chassis_move_t *chassis_move_mode)
{
    ...
    //use controller to set chassis behaviour mode
    //遥控器设置模式
    //6/8/2024 changed move mode to CHASSIS_SPIN when switch is up
    // changed move mode to CHASSIS_ZERO_FORCE when switch is down
    ...
    if (switch_is_mid(chassis_move_mode->chassis_RC->rc.s[CHASSIS_MODE_CHANNEL]))
    {
        ...
        chassis_behaviour_mode = CHASSIS_NO_FOLLOW_YAW; //gimbal independent
    }
    ...
    //根据行为模式选择一个底盘控制模式
    // 6.8 Added SPIN mode at the bottom
...
static void chassis_spin_control(fp32 *vx_set, fp32 *vy_set, fp32 *angle_set, chassis_move_t *chassis_move_rc_to_vector){
    ...
    *angle_set = 0; //yaw motor angle is independent to the chassis
